...
async function taskTester() {
  ...
  async function trippleClickThrough() {
    ...
    if (type != null) {
      ...
      log(
        ...
        chalk.redBright(
          "\n" +
          "This task is not set up to use the task tester," +
            "\n" +
            "Please add necessary variables to code." +
            "\n" +
          "Instructions can be found in the" 
        ) +
        chalk.greenBright(" Powers lab online tasks - Documentation")+
        chalk.yellowBright("\n" + "**********************************"
        )
        );
  ...
  async function buttonPressLogic() {
    ...
    page.on("console", async (e) => {
      ...
      const thresh = await page.evaluate(() => localStorage.getItem("thresh"));
      ...
      if (type == 1) {
        ...
        if (e.text() === "choice") {
          ...
          log(
            chalk.greenBright(
              `Sound was present on the ${session}${
                session == 1 ? "st" : "nd"
              } choice`
            )
          );
